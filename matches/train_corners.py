...
# -*- coding: utf-8 -*-
"""
Corners trainer (totals-only, no minute labels):
- Robust totals extractor across common vendor schemas
- Poisson GLM (goals μ -> corners μ) with safe fallbacks
- NB dispersion with grouped shrinkage (k-min/k-max clamps)
- Exact totals (independent) or Gaussian copula (ρ)
- Optional isotonic calibration (totals + team totals)
- If train has no labels but val does, borrow val for fitting (not for calibration)
- ***Important***: labels are taken ONLY from finished matches (FT/AET/PEN/AFTER_PEN)
"""

# ---------------- Django bootstrap (must be first!) ----------------
import os
if not os.environ.get("DJANGO_SETTINGS_MODULE"):
    os.environ["DJANGO_SETTINGS_MODULE"] = "prediction.settings"  # adjust if needed
...
def _has_corner_word(s: str) -> bool:
    t = _norm(s)
    return ("corner" in t) or ("corners" in t) or ("cornerkicks" in t) or ("cornerkick" in t)
...
def _extract_from_statistics(obj: Any) -> Tuple[Optional[int], Optional[int]]:
    ...
    for k, v in obj.items():
        nk = _norm(k)
        if nk in ("statistics","matchstatistics","teamstatistics","statistic","stats"):
    ...
    for item in stats:
        if not isinstance(item, dict):
            continue
        label = item.get("type") or item.get("name") or item.get("title") or item.get("stat") or item.get("label")
...
def _extract_from_home_away_blocks(obj: Any) -> Tuple[Optional[int], Optional[int]]:
    ...
    def _get_block(d: Dict[str, Any], keys: Tuple[str, ...]) -> Optional[Dict[str, Any]]:
        for k, v in d.items():
    ...
    if isinstance(obj, dict):
        if any(_has_corner_word(k) for k in obj.keys()):
            ...
            for hk in _HOME_KEYS + ("home",):
                if hk in obj:
                    hv = _int0_30(obj.get(hk))
                    break
            for ak in _AWAY_KEYS + ("away",):
                if ak in obj:
                    av = _int0_30(obj.get(ak))
                    break
            if hv is not None or av is not None:
                return (hv, av)
            for v in obj.values():
...
def extract_corner_totals(stats_like: Any) -> Tuple[Optional[int], Optional[int]]:
    if stats_like is None:
        return (None, None)
    h, a = _extract_from_statistics(stats_like)
    if h is not None or a is not None:
        return (h, a)

    def _dfs_blocks(x: Any) -> Tuple[Optional[int], Optional[int]]:
...
def _is_finished_status(s: Optional[str]) -> bool:
    if s is None:
        return False
    return str(s).upper() in _FINISHED_STATUSES

def load_rows(leagues: List[int], seasons: List[int]) -> pd.DataFrame:
    """
    Load rows and attach corner labels. Priority within a *finished* match:
      1) Match.corners_home/away (materialized by ingest)
      2) MatchStats (both teams)
      3) stats10_json extractor (if needed)
    Labels are set to None for non-finished matches to avoid 0-0 leakage.
    Also: if (0,0) but no MatchStats exist, treat as missing.
    """
    recs = []
    missing_both = 0
    ...
    if HAVE_MLTRAIN:
        ...
        for r in base_iter:
            ...
            # Pull Match to know status and teams
            try:
                m = Match.objects.only(
                    "id","home_id","away_id","status","corners_home","corners_away"
                ).get(pk=int(r.fixture_id))
            ...
            if finished and m:
                ...
                # 3) stats10_json crawl (only accept if finished)
                ...
                # Safety: (0,0) with no stats rows → treat as missing
                if (ch == 0 and ca == 0) and not stats_exists:
                    ch, ca = None, None
            else:
                # Not finished -> no labels
                ch, ca = None, None
        ...
        # No MLTrainingMatch table: use Match + MatchStats only
        ...
        for m in base_iter:
            ...
            if finished:
                ...
                # Try raw_result_json crawl only if finished
...
def _label_side_stats(df: pd.DataFrame, side: str, tag: str):
    ...
    head = vc.iloc[:11] if vc.index.min() >= 0 and vc.index.max() <= 30 else vc.head(10)
...
def fit_corner_means(train: pd.DataFrame,
    ...
    print(f"[DBG] goals μ medians: home≈{(muH_med if np.isfinite(muH_med) else float('nan')):.3f} "
          f"away≈{(muA_med if np.isfinite(muA_med) else float('nan')):.3f}  (NaN share={(1.0 - good_mu.mean() if good_mu.size else 1.0):.2%})")
    ...
    if (not labels_ok or not mu_ok) and not force_glm:
        ...
        def val_loss(alpha, beta):
            ...
            for _, r in df_val.loc[mask].iterrows():
                ...
                pmfH = nb_pmf_vec(mH, kH, 30)
                pmfA = nb_pmf_vec(mA, kA, 30)
        ...
        alpha, beta = best
        print(f"[WARN] Using heuristic mapping (α={alpha:.2f}, β={beta:.2f}, mu_bar={mu_bar:.3f})")
        return {
            "kind": "heuristic",
    ...
    # --- GLM fit (Poisson regression on log μ)
    def _fit_side(y_col, own_mu_col, opp_mu_col):
        ...
        print(f"[DBG] GLM mask count for {y_col}: {int(mask.sum())} / {len(mask)} (finite y & μown & μopp)")
        if mask.sum() < 50:
            intercept = math.log(prior_home if y_col.endswith("home") else prior_away)
    ...
    coef_home, mu_hat_h = _fit_side("corners_home", "mu_goals_home", "mu_goals_away")
    coef_away, mu_hat_a = _fit_side("corners_away", "mu_goals_away", "mu_goals_home")
    ...
    def _scale(obs_mean, mu_hat, tag, side_ok: bool):
        ...
        print(f"[DBG] anchor {tag}: observed≈{(obs_mean if np.isfinite(obs_mean) else float('nan')):.2f}, "
...
def predict_means(row: pd.Series, mapping: Dict[str, Any], mean_floor: float = 0.8) -> Tuple[float, float]:
    eps = 1e-6
    def _clip(x):
        return float(np.clip(x, mean_floor, 15.0))
...
def main():
    ...
    ap.add_argument("--total-lines", default="7.5,8.5,9.5,10.5")
    ap.add_argument("--team-lines", default="3.5,4.5")
    ...
    ap.add_argument("--mean-floor", type=float, default=0.8)
    ap.add_argument("--rho-grid", default="0.0")
    ap.add_argument("--sims", type=int, default=20000)
    ap.add_argument("--k-min", type=float, default=5.0)
    ...
    totals_lines = parse_float_list(args.total_lines)
    team_lines   = parse_float_list(args.team_lines)
    hcp_lines    = parse_float_list(args.handicap_lines)

    # -------- Load data
    print("[INFO] Loading rows (totals-only labels)...")
    ...
    # Attach goals μ (if artifact & feature builder exist)
    ...
    print(f"[INFO] Labeled corner pairs: train={nlab_tr}/{nt_tr} (0+0={z_tr}), "
          f"val={nlab_va}/{nt_va} (0+0={z_va}), test={nlab_te}/{nt_te} (0+0={z_te})")

    # Borrow val rows if train has no labels (fit only; not for calibration)
    df_fit = df_train
    ...
    _label_side_stats(df_train, "home", "train"); _label_side_stats(df_train, "away", "train")
    _label_side_stats(df_val,   "home", "val");   _label_side_stats(df_val,   "away", "val")

    # -------- Fit mapping
    mapping = fit_corner_means(
        df_fit,
        df_val,  # validation is used for heuristic tuning and calibration balance
    ...
    kH_global, kH_map = estimate_nb_k_grouped(df_fit, "corners_home",
                                              clip_lo=args.k_min, clip_hi=args.k_max)
    kA_global, kA_map = estimate_nb_k_grouped(df_fit, "corners_away",
                                              clip_lo=args.k_min, clip_hi=args.k_max)
    ...
    def _pmfs_for_row(r):
        mH, mA = predict_means(r, mapping, mean_floor=args.mean_floor)
        pmfH = nb_pmf_vec(mH, k_for(r, "H"), args.nmax)
        pmfA = nb_pmf_vec(mA, k_for(r, "A"), args.nmax)
        return pmfH, pmfA
    ...
    def _val_totals_logloss(rho: float) -> float:
        ...
        for _, r in df_val.loc[mask].iterrows():
            ...
            tot = conv_sum(pmfH, pmfA, args.nmax) if abs(rho) < 1e-9 else totals_pmf_copula(pmfH, pmfA, rho=rho, sims=args.sims, seed=int(r["fixture_id"]), nmax=args.nmax)[0]
            line_losses = []
    ...
    if mask_val.any():
        ...
        for _, r in rows_val.iterrows():
            pmfH, pmfA = _pmfs_for_row(r)
            tot = conv_sum(pmfH, pmfA, args.nmax) if abs(chosen_rho) < 1e-9 else totals_pmf_copula(pmfH, pmfA, rho=chosen_rho, sims=args.sims, seed=int(r["fixture_id"]), nmax=args.nmax)[0]
            ...
            for L in team_lines:
                thr = math.floor(L + 1e-9) + 1
                idxH = min(thr-1, args.nmax); idxA = min(thr-1, args.nmax)
    ...
    for _, r in df_test.iterrows():
        ...
        mH, mA = predict_means(r, mapping, mean_floor=args.mean_floor)
        out["m_corners_home"] = float(mH)
        out["m_corners_away"] = float(mA)
        ...
        if abs(chosen_rho) < 1e-9:
            ...
            _, sim_h, sim_a = totals_pmf_copula(pmfH, pmfA, rho=chosen_rho, sims=args.sims, seed=int(r["fixture_id"]), nmax=args.nmax)
            D = sim_h - sim_a
            for h in hcp_lines:
                if float(h).is_integer():
                    h_int = int(h)
                    p_win  = float(np.mean(D >  h_int))
                    p_push = float(np.mean(D == h_int))
                    p_lose = float(1.0 - p_win - p_push)
                else:
                    p_win  = float(np.mean(D > h))
                    p_push = 0.0
                    p_lose = float(1.0 - p_win)
                out[f"hcp_home_{h}_win"]  = p_win
                out[f"hcp_home_{h}_push"] = p_push
    ...
    artifacts = {
        "type": "corners_nb_totals_only_v3",
        "family": "nb",
        "mapping": mapping,
        ...
        "totals_calibration": cal_totals,
        "team_calibration": {"home": cal_team_home, "away": cal_team_away},
        ...
        "metrics": metrics,
        "notes": "Finished-only labels; multi-schema extractor; borrowed-val fit when train has no labels.",
    }
    ...
    if not preds.empty:
        print("[DBG] sample means (test head):",
              f'home≈{preds["m_corners_home"].head().mean():.2f},',
              f'away≈{preds["m_corners_away"].head().mean():.2f}')
        print("[DBG] unique mean counts:",
              f'H={preds["m_corners_home"].nunique()} A={preds["m_corners_away"].nunique()}')
