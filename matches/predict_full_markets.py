...
def _safe_blend_btts_calibrator(
    ...
    uniq = int(np.unique(np.round(ys, 3)).size)
    ...
    w_small = _scale(lsmall, BTTS_LOCAL_SPREAD_MIN_SMALL, 0.06)
    w_big   = _scale(lbig,   BTTS_LOCAL_SPREAD_MIN_BIG,   0.15)
    w_global= _scale(gspread, BTTS_GLOBAL_SPREAD_MIN,     0.30)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--rho", type=float, default=DEFAULT_RHO,
                            help="Coupling for correlated grid (sqrt(lh*la) scaling).")
        parser.add_argument("--rho-1x2", type=float, default=None,
                            help="Override rho only for 1X2; default = --rho. Use 0 to avoid draw inflation.")
        ...
        parser.add_argument("--draw-shrink", type=float, default=0.0,
                            help="Shrink draw prob by this fraction (0..0.5) and redistribute to H/A proportionally.")
    ...
    def handle(self, *args, **opts):
        ...
        rho_1x2_in = opts.get("rho_1x2")
        rho_1x2 = float(np.clip(rho if rho_1x2_in is None else rho_1x2_in, 0.0, DEFAULT_RHO_MAX))
        ...
        draw_shrink: float = float(np.clip(opts.get("draw_shrink", 0.0), 0.0, 0.5))
        ...
        def _grid_for_1x2(lh: float, la: float) -> np.ndarray:
            # Use a controlled rho specifically for 1X2 to avoid draw inflation from correlation.
            d = abs(lh - la)
            taper = math.exp(-c_taper * d) if c_taper > 0 else 1.0
            return _bp_grid_rho(lh, la, rho_1x2 * taper, max_goals)
        ...
        for mp in qs:
            ...
            # 1X2 from separate grid with its own rho (often 0.0 to cut draw inflation)
            P_1x2 = _grid_for_1x2(use_lh, use_la)
            pH, pD, pA = _one_x_two_from_grid(P_1x2)

            # Optional draw shrink (reallocate to H & A proportionally)
            if draw_shrink > 0.0:
                deltaD = pD * draw_shrink
                denom = (pH + pA)
                if denom > 0:
                    pH += deltaD * (pH / denom)
                    pA += deltaD * (pA / denom)
            ...
            # Optional per-leg 1X2 calibration (keys: 1x2_H, 1x2_D, 1x2_A) then renormalize
            pHc = _apply_calibrator(calibrators, "1x2_H", pH)
            pDc = _apply_calibrator(calibrators, "1x2_D", pD)
            pAc = _apply_calibrator(calibrators, "1x2_A", pA)
            s_cal = pHc + pDc + pAc
            if s_cal > 0 and np.isfinite(s_cal):
                pH, pD, pA = pHc/s_cal, pDc/s_cal, pAc/s_cal
            ...
            # HT/FT (split λs; correlated halves for realism)
            ...
            if verbose_matches:
                self.stdout.write(
                    ...
                    f"[ρ1x2={rho_1x2:.2f}, shrink={draw_shrink:.2f}] "
