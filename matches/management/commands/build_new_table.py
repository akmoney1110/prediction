# matches/management/commands/build_ml_rows_slim.py
"""
...
- Builds per-team history windows as-of each fixture:
    ...
    * simple situational      (rest-days, matches in last 7/14 days, b2b flag)
...
try:
    ...
    MatchEvent = None  # pragma: no cover
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opt):
        ...
        self.stdout.write(self.style.SUCCESS("[build_ml_rows_slim] Done."))
    ...
    def _process_fixture(self, m: Match):
        ...
        # Simple cross features (one per side; readable)
        ...
        # Backfill corners/cards from stats when match fields are None
        if (y_hc is None or y_ac is None) or (y_hcd is None or y_acd is None):
            rows = self.stats_by_match.get(m.id, [])
            hrow = next((r for r in rows if r.team_id == m.home_id), None)
            arow = next((r for r in rows if r.team_id == m.away_id), None)
            if y_hc is None and hrow and arow and hrow.corners is not None and arow.corners is not None:
                y_hc, y_ac = hrow.corners, arow.corners
            if y_hcd is None and hrow and arow and hrow.cards is not None and arow.cards is not None:
                y_hcd, y_acd = hrow.cards, arow.cards
    ...
    def _count_stats_rows(self, team_id: int, matches: List[Match]) -> int:
        ...
        for mm in matches:
            ...
            if any(r.team_id == team_id for r in rows):
                cnt += 1
        return cnt
    ...
    def _agg_means(self, team_id: int, matches: List[Match], alpha: float) -> Dict[str, float]:
        ...
        for mm in matches:
            ...
            st = next((r for r in self.stats_by_match.get(mm.id, []) if r.team_id == team_id), None)
            if st:
    ...
    def _agg_allowed(self, team_id: int, matches: List[Match], cutoff, weighted_w: Optional[List[float]] = None) -> Dict[str, float]:
        ...
        for i, mm in enumerate(matches):
            opp = next((r for r in self.stats_by_match.get(mm.id, []) if r.team_id != team_id), None)
            if not opp:
                continue
            ...
            # xg is opponent's attacking xg; treat as xGA for our team
