...
class Command(BaseCommand):
    help = "Train football prediction model from merged dataset using HomeWin as target"
    ...
    def handle(self, *args, **kwargs):
        ...
        if 'HomeWin' not in df.columns:
            self.stdout.write(self.style.ERROR("❌ 'HomeWin' column missing for target creation"))
            return
        df['target'] = df['HomeWin']

        # Drop columns that leak match outcome or are irrelevant
        drop_cols = [
            'FTHG', 'FTAG', 'HTHG', 'HTAG', 'HTR', 'HomeWin', 'SourceFile', 'Date', 'Time'
        ]
